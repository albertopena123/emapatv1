...
const EmapatLanding = () => {
  ...
  // Update state declarations with types
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      signalWaves.children.forEach((wave, index) => {
        ...
        (mesh.material as THREE.MeshBasicMaterial).opacity = Math.max(0.1, opacity);
      });
      ...
      dataFlow.children.forEach((particle, index) => {
        ...
        if (particle instanceof THREE.Mesh) {
          (particle.material as THREE.MeshBasicMaterial).opacity = Math.sin(progress * Math.PI) * 0.8;
        }
      });

      // Animate tower light pulsing
      scene.traverse((child) => {
        if (child.userData.isTowerLight && child instanceof THREE.Mesh) {
          const scale = 1 + Math.sin(time * child.userData.pulseSpeed * 100) * 0.5;
          child.scale.setScalar(scale);

          const intensity = 0.5 + Math.sin(time * 6) * 0.3;
          (child.material as THREE.MeshPhongMaterial).emissive.setScalar(intensity);
        }
      });

      // Animate LCD display on meter (changing numbers effect)
      if (smartMeter.children[1] instanceof THREE.Mesh) {
        const intensity = 0.3 + Math.sin(time * 3) * 0.2;
        (smartMeter.children[1].material as THREE.MeshPhongMaterial).emissive.setScalar(intensity);
      }
      ...
      endDevices.children.forEach((device, index) => {
        if (device instanceof THREE.Mesh && device.material && 'emissive' in device.material) {
          const intensity = 0.2 + Math.sin(time * 4 + index * Math.PI) * 0.15;
          (device.material as THREE.MeshPhongMaterial).emissive.setScalar(intensity);
        }
      });
  ...
  return (
    <div className={`min-h-screen overflow-x-hidden transition-all duration-500 relative ${isDarkMode ? 'bg-black text-white' : 'bg-white text-gray-900'
      }`}>
      ...
      <div className="fixed inset-0 pointer-events-none z-10 overflow-hidden">
        {waterDrops.map((drop) => (
          <div
            key={drop.id}
            className={`absolute rounded-full animate-water-drop ${isDarkMode ? 'bg-cyan-400/20' : 'bg-blue-500/30'
              }`}
            style={{
              left: `${drop.left}%`,
              width: `${drop.width}px`,
      ...
      <div className="fixed inset-0 pointer-events-none z-5 overflow-hidden">
        {floatingParticles.map((particle) => (
          <div
            key={particle.id}
            className={`absolute w-1 h-1 rounded-full animate-float-particle ${isDarkMode ? 'bg-cyan-300/40' : 'bg-blue-400/50'
              }`}
            style={{
              left: `${particle.left}%`,
              top: `${particle.top}%`,
      ...
      {/* Progress Bar */}
      <div className={`fixed top-0 left-0 w-full h-1 z-50 transition-colors duration-500 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-200'
        }`}>
        <div
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 w-full backdrop-blur-xl z-40 border-b transition-all duration-500 ${isDarkMode
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className={`text-2xl font-bold transition-colors duration-500 ${isDarkMode
            ...
            <div className="hidden md:flex items-center space-x-8">
              <a href="#inicio" className={`font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-300 hover:text-cyan-400' : 'text-gray-700 hover:text-blue-600'
                }`}>Inicio</a>
              <a href="#tecnologia" className={`font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-300 hover:text-cyan-400' : 'text-gray-700 hover:text-blue-600'
                }`}>Tecnología</a>
              <a href="#soluciones" className={`font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-300 hover:text-cyan-400' : 'text-gray-700 hover:text-blue-600'
                }`}>Soluciones</a>
              <a href="#contacto" className={`font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-300 hover:text-cyan-400' : 'text-gray-700 hover:text-blue-600'
                }`}>Contacto</a>
            ...
            <button
              className={`md:hidden p-2 transition-colors ${isDarkMode ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-gray-900'
                }`}
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className={`fixed inset-0 z-30 pt-20 md:hidden transition-all duration-300 ${isDarkMode ? 'bg-black' : 'bg-white'
          }`}>
          <div className="px-6 py-8 space-y-6">
            <a href="#inicio" className={`block py-3 font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-300 hover:text-cyan-400' : 'text-gray-700 hover:text-blue-600'
              }`}>Inicio</a>
            <a href="#tecnologia" className={`block py-3 font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-300 hover:text-cyan-400' : 'text-gray-700 hover:text-blue-600'
              }`}>Tecnología</a>
            <a href="#soluciones" className={`block py-3 font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-300 hover:text-cyan-400' : 'text-gray-700 hover:text-blue-600'
              }`}>Soluciones</a>
            <a href="#contacto" className={`block py-3 font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-300 hover:text-cyan-400' : 'text-gray-700 hover:text-blue-600'
              }`}>Contacto</a>

            <button
              onClick={() => setIsDarkMode(!isDarkMode)}
              className={`flex items-center w-full py-3 px-4 rounded-lg transition-all duration-300 ${isDarkMode
                ? 'bg-gray-800 hover:bg-gray-700 text-yellow-400'
                : 'bg-gray-100 hover:bg-gray-200 text-gray-700'
                }`}
            >
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0 opacity-20">
            {heroParticles.map((particle) => (
              <div
                key={particle.id}
                className={`absolute w-1 h-1 rounded-full animate-pulse ${isDarkMode ? 'bg-cyan-400' : 'bg-blue-500'
                  }`}
                style={{
                  left: `${particle.left}%`,
                  top: `${particle.top}%`,
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
          <div className={`space-y-10 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
            <div className="space-y-8">
              <div className={`inline-flex items-center border px-6 py-3 rounded-full text-sm font-semibold backdrop-blur-sm transition-all duration-500 ${isDarkMode
              ...
              <h1 className="text-6xl lg:text-8xl font-black leading-tight">
                <span className={`bg-clip-text text-transparent animate-gradient transition-all duration-500 ${isDarkMode
              ...
              <p className={`text-2xl leading-relaxed max-w-lg transition-colors duration-500 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
                }`}>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              ...
              <button className={`group backdrop-blur-sm border-2 px-10 py-5 rounded-full transition-all duration-500 font-bold text-lg flex items-center justify-center ${isDarkMode
            ...
            <div className="grid grid-cols-3 gap-8 pt-12">
              <div className="text-center group">
                <div className={`text-4xl font-black mb-2 transition-colors duration-300 ${isDarkMode
                  ? 'text-white group-hover:text-cyan-400'
                  : 'text-gray-900 group-hover:text-blue-600'
                  }`}>
                  15,847
                </div>
                <div className={`text-sm font-medium transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                  }`}>Medidores IoT</div>
              </div>
              <div className="text-center group">
                <div className={`text-4xl font-black mb-2 transition-colors duration-300 ${isDarkMode
          ...
          <div className={`relative ${isVisible ? 'animate-fade-in-right' : 'opacity-0'}`}>
            <div className="relative">
              <div
                ref={threejsRef}
                className={`w-full h-[700px] rounded-3xl shadow-2xl overflow-hidden border transition-all duration-500 ${isDarkMode
              ...
              <div className={`absolute top-8 left-8 backdrop-blur-sm rounded-2xl p-6 border animate-float transition-all duration-500 ${isDarkMode
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className={`font-bold text-lg transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-900'
                      }`}>Sistema Activo</div>
              ...
              <div className={`absolute bottom-8 right-8 backdrop-blur-sm rounded-2xl p-6 border animate-float-delayed transition-all duration-500 ${isDarkMode
              ...
              <div className={`absolute top-1/2 -right-4 backdrop-blur-sm rounded-2xl p-4 border animate-pulse transition-all duration-500 ${isDarkMode
                ? 'bg-black/60 border-purple-500/30'
                : 'bg-white/80 border-purple-200'
                }`}>
                <div className="flex items-center space-x-2">
                  <Zap className="w-5 h-5 text-yellow-500" />
                  <span className={`font-semibold transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-900'
                    }`}>IA Activa</span>
                </div>
              </div>
            </div>
      ...
      {/* Features Section */}
      <section className={`py-32 relative transition-colors duration-500 ${isDarkMode ? 'bg-gradient-to-b from-black to-gray-900' : 'bg-gradient-to-b from-white to-gray-50'
        }`}>
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="text-center mb-20">
            <h2 className={`text-5xl lg:text-6xl font-black mb-8 transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-900'
              }`}>
            ...
            <p className={`text-xl max-w-3xl mx-auto transition-colors duration-500 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
              }`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <div
                key={index}
                className={`group backdrop-blur-sm rounded-3xl p-8 border transition-all duration-500 hover:transform hover:scale-105 hover:shadow-2xl hover:shadow-cyan-500/20 ${isDarkMode
              ...
              >
                ...
                <h3 className={`text-2xl font-bold mb-6 transition-colors ${isDarkMode
                  ? 'text-white group-hover:text-cyan-400'
                  : 'text-gray-900 group-hover:text-blue-600'
                  }`}>
                  {feature.title}
                </h3>
                <p className={`leading-relaxed text-lg transition-colors duration-500 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
                  }`}>
                  {feature.description}
                </p>
              </div>
      ...
      {/* CTA Section */}
      <section className={`py-32 relative transition-colors duration-500 ${isDarkMode ? 'bg-black' : 'bg-gray-50'
        }`}>
        <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
          <div className="space-y-12">
            <h2 className={`text-6xl lg:text-7xl font-black leading-tight transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-900'
              }`}>
            ...
            <p className={`text-2xl max-w-3xl mx-auto leading-relaxed transition-colors duration-500 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
              }`}>
            ...
            <div className={`pt-12 flex justify-center items-center space-x-12 transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
              }`}>
      ...
      {/* Footer */}
      <footer className={`border-t py-16 transition-all duration-500 ${isDarkMode
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
            <div>
              <div className="flex items-center space-x-4 mb-8">
                ...
                <div>
                  <h1 className={`text-2xl font-bold transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-900'
                    }`}>EPS EMAPAT SA</h1>
                  <p className={`transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                    }`}>Smart Water IoT Solutions</p>
                </div>
              </div>
              <p className={`leading-relaxed transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
            ...
            <div>
              <h3 className={`font-bold text-lg mb-6 transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-900'
                }`}>Tecnología</h3>
              <ul className={`space-y-3 transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
                <li className={`transition-colors cursor-pointer ${isDarkMode ? 'hover:text-cyan-400' : 'hover:text-blue-600'
                  }`}>IoT Avanzado</li>
                <li className={`transition-colors cursor-pointer ${isDarkMode ? 'hover:text-cyan-400' : 'hover:text-blue-600'
          ...
          <div className={`border-t pt-8 text-center transition-all duration-500 ${isDarkMode
